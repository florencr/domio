...
export async function PATCH(
  ...
  try {
    ...
    const { name, address, vat_account } = await request.json();

    const updates: { name?: string; address?: string; vat_account?: string } = {};
    if (name !== undefined) updates.name = name.trim();
    if (address !== undefined) updates.address = address.trim();
    if (vat_account !== undefined) updates.vat_account = vat_account?.trim() || null;

    if (Object.keys(updates).length === 0) {
      return NextResponse.json({ error: "name, address or vat_account required" }, { status: 400 });
    }
