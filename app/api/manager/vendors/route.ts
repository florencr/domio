...
export async function GET() {
  try {
    ...
    const { data: vendors, error } = await admin.from("vendors").select("id,name,site_id");
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });

    const filtered = (vendors ?? []).filter((v: { site_id?: string | null }) => !v.site_id || v.site_id === siteId);
...
export async function PATCH(request: Request) {
  try {
    ...
    const { data: existing } = await admin.from("vendors").select("site_id").eq("id", id).single();
    if (!existing) return NextResponse.json({ error: "Not found" }, { status: 404 });
    if (existing.site_id && existing.site_id !== siteId) return NextResponse.json({ error: "Forbidden" }, { status: 403 });

    const { data, error } = await admin.from("vendors").update({ name: name.trim() }).eq("id", id).select("id,name,site_id").single();
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
    return NextResponse.json(data);
  } catch (err) {
    return NextResponse.json({ error: err instanceof Error ? err.message : "Unknown error" }, { status: 500 });
