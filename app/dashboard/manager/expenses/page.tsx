...
export default function ManagerExpensesPage() {
  ...
  async function generateRecurrentExpenses() {
    ...
    try {
      ...
      if (res.ok && j.success) {
        setMsg({ text: `✓ Generated ${j.count ?? 0} recurrent expense(s) for ${MONTHS[m - 1]} ${y}.`, ok: true });
        load();
  ...
  async function deleteExpensesPeriod() {
    ...
    const j = await res.json().catch(() => ({}));
    if (res.ok && j.success) {
      setMsg({ text: `✓ Deleted ${j.expensesDeleted ?? 0} recurrent expense(s) for ${MONTHS[m - 1]} ${y}.`, ok: true });
      load();
    } else setMsg({ text: j.error || "Failed", ok: false });
  }

  async function recordAdhoc() {
    if (!adhocForm.title.trim()) { setMsg({ text: "Title is required", ok: false }); return; }
    ...
    if (res.ok && j.success) {
      ...
      setMsg({ text: j.message ?? "✓ Ad-hoc expense recorded.", ok: true });
  ...
  async function updateExpense(id: string) {
    if (!editForm.title.trim()) { setMsg({ text: "Title is required", ok: false }); return; }
    ...
    setSaving(false);
    if (res.ok && j.success) {
      setMsg({ text: "✓ Expense updated.", ok: true });
      setEditingId(null);
      load();
    } else setMsg({ text: j.error || "Failed", ok: false });
  }
  ...
  async function deleteExpense(id: string) {
    ...
    setSaving(false);
    if (res.ok && j.success) {
      setMsg({ text: "✓ Expense deleted.", ok: true });
      setEditingId(null);
      load();
    } else setMsg({ text: j.error || "Failed", ok: false });
  }
    ...
    <div className="space-y-4 mt-2">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <Card className="border-l-4 border-l-blue-500 py-3 gap-1 px-4">
          ...
          <div>
            <p className="text-xs font-bold text-muted-foreground uppercase tracking-wide">Monthly Recurrent (Templates)</p>
        ...
        <Card className="border-l-4 border-l-amber-500 py-3 gap-1 px-4">
          ...
          <div>
            <p className="text-xs font-bold text-muted-foreground uppercase tracking-wide">Ad-hoc</p>
      ...
      <Card>
        <CardHeader className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pb-2">
          <CardTitle className="text-base sm:text-lg">All Expenses ({sortedExpenses.length}{filteredExpenses.length !== data.expenses.length ? ` of ${data.expenses.length}` : ""})</CardTitle>
        ...
        <CardContent className="space-y-3 pt-0">
          {showRecurrentForm && (
            <div className="rounded-md border-l-4 border-emerald-400 bg-emerald-50/60 dark:bg-emerald-950/20 p-3 space-y-2">
              <p className="text-xs font-medium text-emerald-800 dark:text-emerald-200">Generate recurrent expenses for a period (copies templates from Config &gt; Expenses).</p>
              <div className="flex flex-wrap gap-2 items-center">
                ...
                <Button size="sm" className="h-8" onClick={generateRecurrentExpenses} disabled={generating}>{generating ? "..." : "Generate"}</Button>
                <Button variant="outline" size="sm" className="h-8" disabled={!isPeriodCurrent(parseInt(genMonth), parseInt(genYear))} onClick={deleteExpensesPeriod}>Delete period</Button>
                <Button variant="ghost" size="sm" className="h-8" onClick={() => setShowRecurrentForm(false)}>Cancel</Button>
              </div>
              <div className="text-xs text-muted-foreground">
                <strong>Recurrent templates:</strong> {data.expenses.filter(e => e.period_month == null && e.period_year == null && e.frequency === "recurrent").length}
              </div>
            </div>
          )}
          {showAdhocForm && (
            <div className="rounded-md border-l-4 border-emerald-500 bg-emerald-50/60 dark:bg-emerald-950/20 p-3 space-y-2">
              <p className="text-xs font-medium text-emerald-800 dark:text-emerald-200">Record one-off expense for a specific period.</p>
              <div className="flex flex-wrap gap-2 items-end">
                <div className="min-w-0 flex-1"><Label className="text-xs">Title</Label><Input value={adhocForm.title} onChange={e => setAdhocForm({ ...adhocForm, title: e.target.value })} placeholder="e.g. Extra repair" className="h-8" /></div>
                <div><Label className="text-xs">Category</Label><Select value={adhocForm.category || "Misc"} onValueChange={v => setAdhocForm({ ...adhocForm, category: v })}><SelectTrigger className="h-8 w-28"><SelectValue /></SelectTrigger><SelectContent>{adhocCategoryOptions.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent></Select></div>
                <div><Label className="text-xs">Vendor</Label><Select value={adhocForm.vendor || "__none__"} onValueChange={v => setAdhocForm({ ...adhocForm, vendor: v === "__none__" ? "" : v })}><SelectTrigger className="h-8 w-28"><SelectValue placeholder="—" /></SelectTrigger><SelectContent><SelectItem value="__none__">—</SelectItem>{adhocVendorOptions.filter(v => v !== "—").map(v => <SelectItem key={v} value={v}>{v}</SelectItem>)}</SelectContent></Select></div>
                <div><Label className="text-xs">Amount €</Label><Input type="number" step="0.01" value={adhocForm.amount} onChange={e => setAdhocForm({ ...adhocForm, amount: e.target.value })} placeholder="0.00" className="h-8 w-24" /></div>
                <div><Label className="text-xs">Period</Label><Select value={adhocForm.periodM} onValueChange={v => setAdhocForm({ ...adhocForm, periodM: v })}><SelectTrigger className="h-8 w-24"><SelectValue /></SelectTrigger><SelectContent>{MONTHS.map((m, i) => <SelectItem key={i} value={String(i + 1)}>{m}</SelectItem>)}</SelectContent></Select></div>
                <div><Label className="text-xs">Year</Label><Select value={adhocForm.periodY} onValueChange={v => setAdhocForm({ ...adhocForm, periodY: v })}><SelectTrigger className="h-8 w-20"><SelectValue /></SelectTrigger><SelectContent>{[new Date().getFullYear(), new Date().getFullYear() - 1].map(y => <SelectItem key={y} value={String(y)}>{y}</SelectItem>)}</SelectContent></Select></div>
                <Button size="sm" className="h-8" onClick={recordAdhoc} disabled={saving}>{saving ? "..." : "Save"}</Button>
                <Button variant="ghost" size="sm" className="h-8" onClick={() => setShowAdhocForm(false)}>Cancel</Button>
              </div>
            </div>
          )}
          <div className={`grid transition-[grid-template-rows] duration-200 ${showFilters ? "grid-rows-[1fr]" : "grid-rows-[0fr]"} md:grid-rows-[1fr]`}>
            <div className="min-h-0 overflow-hidden">
              <div className="flex flex-wrap gap-2 items-end pb-3">
                <div><Label className="text-xs">Period</Label><Select value={filterPeriod} onValueChange={setFilterPeriod}><SelectTrigger className="h-8 w-32"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All periods</SelectItem>{periodOptions.map(k => { const [y, m] = k.split("-"); return <SelectItem key={k} value={k}>{MONTHS[parseInt(m || "1") - 1]} {y}</SelectItem> })}</SelectContent></Select></div>
                <div><Label className="text-xs">Category</Label><Select value={filterCategory} onValueChange={setFilterCategory}><SelectTrigger className="h-8 w-32"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All</SelectItem>{categories.map(c => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent></Select></div>
                <div><Label className="text-xs">Vendor</Label><Select value={filterVendor} onValueChange={setFilterVendor}><SelectTrigger className="h-8 w-32"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All</SelectItem>{vendors.map(v => <SelectItem key={v} value={v}>{v}</SelectItem>)}</SelectContent></Select></div>
                <div><Label className="text-xs">Frequency</Label><Select value={filterFrequency} onValueChange={setFilterFrequency}><SelectTrigger className="h-8 w-28"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="all">All</SelectItem><SelectItem value="recurrent">Recurrent</SelectItem><SelectItem value="ad_hoc">Ad-hoc</SelectItem></SelectContent></Select></div>
              </div>
            </div>
          </div>
            ...
            <table className="w-full text-sm min-w-[600px]">
              <thead>
                <tr className="border-b text-left">
                  ...
                  <SortableTh column="category" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} className="pb-3 pr-4 font-medium text-muted-foreground">Category</SortableTh>
                  <SortableTh column="title" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} className="pb-3 pr-4 font-medium text-muted-foreground">Title</SortableTh>
                  <SortableTh column="vendor" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} className="pb-3 pr-4 font-medium text-muted-foreground">Vendor</SortableTh>
                  ...
                  <SortableTh column="frequency" sortCol={sortCol} sortDir={sortDir} onSort={handleSort} className="pb-3 pr-4 font-medium text-muted-foreground">Frequency</SortableTh>
              ...
              <tbody className="divide-y divide-border">
                    ...
                    {editingId === e.id ? (
                      <>
                        <td colSpan={9} className="py-2">
                          <div className="flex flex-wrap gap-2 items-end">
                            <Input value={editForm.title} onChange={ev => setEditForm({ ...editForm, title: ev.target.value })} placeholder="Title" className="h-8 w-36" />
                            <Input value={editForm.category} onChange={ev => setEditForm({ ...editForm, category: ev.target.value })} placeholder="Category" className="h-8 w-24" />
                            <Input value={editForm.vendor} onChange={ev => setEditForm({ ...editForm, vendor: ev.target.value })} placeholder="Vendor" className="h-8 w-24" />
                            ...
                            <Select value={editForm.frequency} onValueChange={v => setEditForm({ ...editForm, frequency: v })}><SelectTrigger className="h-8 w-24"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="recurrent">Recurrent</SelectItem><SelectItem value="ad_hoc">Ad-hoc</SelectItem></SelectContent></Select>
                            <Button size="sm" className="h-8" onClick={() => updateExpense(e.id)} disabled={saving}>{saving ? "..." : "Save"}</Button>
                            <Button variant="ghost" size="sm" className="h-8" onClick={() => setEditingId(null)}>Cancel</Button>
                          </div>
                        </td>
                      </>
                    ) : (
                      <>
                        ...
                        <td className="py-3 pr-4"><span className={`inline-block text-[10px] font-medium px-1.5 py-0.5 rounded ${e.frequency === "ad_hoc" ? "bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-400" : "bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-400"}`}>{e.frequency === "ad_hoc" ? "Ad-hoc" : "Recurrent"}</span></td>
                        <td className="py-3 pr-4">{e.paid_at ? <span className="inline-flex items-center gap-1 text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full font-medium">✓ Paid</span> : <span className="inline-flex items-center gap-1 text-xs bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full font-medium">Unpaid</span>}</td>
                        <td className="py-3">
                          <div className="flex flex-wrap gap-1">
                            ...
                            <Button size="sm" variant="ghost" className="h-7 text-xs" onClick={() => e.paid_at ? markUnpaid(e.id) : markPaid(e.id)}>{e.paid_at ? "Mark unpaid" : "Mark paid"}</Button>
                          </div>
                        </td>
                      </>
                    )}
                ...
                {!sortedExpenses.length && <tr><td colSpan={9} className="py-8 text-center text-muted-foreground">{data.expenses.length === 0 ? "No expenses yet. Add a recurrent template or ad-hoc expense." : "No expenses match filters."}</td></tr>}
              </tbody>
            </table>
          </div>
        </CardContent>
